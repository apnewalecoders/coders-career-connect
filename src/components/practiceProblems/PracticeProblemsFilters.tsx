...
const PracticeProblemsFilters = ({ onFiltersChange }: PracticeProblemsFiltersProps) => {
  ...
  const activeFilters = [
    ...
    ...(topic !== "All" ? [{ label: topics.find(t => t.value === topic)?.label || topic, color: "bg-blue-50 border-blue-200 text-blue-800" }] : []),
    ...(company !== "All" ? [{ label: companies.find(c => c.value === company)?.label || company, color: "bg-green-50 border-green-200 text-green-800" }] : [])
  ];
